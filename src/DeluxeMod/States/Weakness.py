...
async def register(root_context: StateContext[Weakness]):
    ...
    async def notify(context: EventContext[PreMoveGameEvent]):
        ...
        target.notifications.append(ls("deluxe.state.weakness.notification").format(state.turns))
    ...
    async def apply_energy_cost_increase(context: EventContext[PreActionsGameEvent]):
        ...
        session.say(ls("deluxe.state.weakness.energy_cost_increase").format(target.name, 1), source_id=target.id, target_id=target.id)
    ...
    async def reset_energy_cost_increase(context: EventContext[PostDamagesGameEvent]):
        ...
        if not state.turns:
            session.say(ls("deluxe.state.weakness.recovery").format(target.name), source_id=target.id, target_id=target.id)
