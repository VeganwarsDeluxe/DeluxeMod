...
async def register(root_context: StateContext[Emptiness]):
    ...
    async def func(context: EventContext[PreDamagesGameEvent]):
        if state.emptiness >= 3:
            session.say(ls("state.emptiness.energy_loss").format(target.name, target.max_energy - 1), source_id=source.id, target_id=source.id)
            target.max_energy -= 1
            state.active = False
            state.emptiness = 1
        elif state.triggered:
            session.say(ls("state.emptiness.timer").format(target.name, max(state.emptiness, 0)), source_id=source.id, target_id=source.id)
