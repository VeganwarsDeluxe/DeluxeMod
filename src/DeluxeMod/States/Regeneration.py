...
async def register(root_context: StateContext[Regeneration]):
    ...
    async def func(context: EventContext[PreDamagesGameEvent]):
        if state.regeneration <= 0:
            ...
            session.say(ls("deluxe.state.regeneration_hp_recovery").format(source.name, source.hp), source_id=source.id, target_id=source.id)
            state.active = False
            state.regeneration = 3
        else:
            session.say(ls("deluxe.state.regeneration_timer").format(source.name, max(state.regeneration, 0)), source_id=source.id, target_id=source.id)
