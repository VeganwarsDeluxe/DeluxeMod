...
class StarBow(RangedWeapon):
    id = 'star_bow'
    name = ls("deluxe.weapon.star_bow.name")
    description = ls("deluxe.weapon.star_bow.description")
    ...
    async def func(self, source, target):
        ...
        if percentage_chance(75):
            if target.get_state(Visor):
                self.session.say(ls("deluxe.weapon.star_bow.visor_fail").format(target.name), source_id=source.id, target_id=target.id)
                return damage

            blindness = target.get_state(Blindness)
            blindness.blind(self.session)
            self.session.say(ls("deluxe.weapon.star_bow.effect").format(target.name), source_id=source.id, target_id=target.id)
